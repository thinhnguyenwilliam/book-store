...
func (h *Handler) RegisterRoutes(e *echo.Echo) {
	...
	api.POST("/auth/oauth/:provider/start", h.startOAuth)
	api.POST("/auth/oauth/:provider/finish", h.finishOAuth)
	...
	secured.GET("/auth/me/permissions", h.getAccess)
	...
	admin.Use(RequirePermission("admin.access"))
	admin.GET("/customers", h.listCustomers, RequirePermission("customers.read"))
	admin.GET("/customers/:id", h.getCustomer, RequirePermission("customers.read"))
	admin.PUT("/customers/:id", h.updateCustomer, RequirePermission("customers.update"))
	admin.DELETE("/customers/:id", h.deleteCustomer, RequirePermission("customers.delete"))
	admin.POST("/books", h.createBook, RequirePermission("books.create"))
	admin.PUT("/books/:id", h.updateBook, RequirePermission("books.update"))
	admin.DELETE("/books/:id", h.deleteBook, RequirePermission("books.delete"))
	admin.PUT("/wallets/:owner_id/balance", h.updateWalletBalance, RequirePermission("wallets.adjust"))
	admin.PUT("/comments/:id/status", h.moderateComment, RequirePermission("comments.moderate"))
	admin.GET("/analytics/orders", h.getOrderAnalytics, RequirePermission("analytics.read"))
	admin.GET("/analytics/customer-activity", h.getCustomerActivityAnalytics, RequirePermission("analytics.read"))
	admin.GET("/roles", h.authorizationCatalog, RequirePermission("roles.read"))
	admin.POST("/roles", h.saveRole, RequirePermission("roles.manage"))
	admin.PUT("/roles/:code", h.saveRole, RequirePermission("roles.manage"))
	admin.DELETE("/roles/:code", h.deleteRole, RequirePermission("roles.manage"))
	admin.GET("/accounts/:id/permissions", h.getAccess, RequirePermission("roles.read"))
	admin.PUT("/accounts/:id/roles", h.assignRoles, RequirePermission("users.assign_roles"))
	admin.GET("/authorization/audit", h.authorizationAudit, RequirePermission("roles.read"))
